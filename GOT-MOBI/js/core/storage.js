...
function loadData() {
    ...
    try {
        const rawMarket = localStorage.getItem('got_market');
        if (rawMarket) marketListings = JSON.parse(rawMarket);
    } catch(e) {
        marketListings = [];
    }
    
    try {
        const rawTrader = localStorage.getItem('got_trader');
        if (rawTrader) traderInventory = JSON.parse(rawTrader);
    } catch(e) {
        traderInventory = {};
    }
    
    try {
        const rawLog = localStorage.getItem('got_log');
        if (rawLog) gameLog = JSON.parse(rawLog);
    } catch(e) {
        gameLog = [];
    }
    
    try {
        const rawConfiscated = localStorage.getItem('got_confiscated');
        if (rawConfiscated) confiscatedItems = JSON.parse(rawConfiscated);
    } catch(e) {
        confiscatedItems = [];
    }
    
    try {
        const rawDice = localStorage.getItem('got_dice_games');
        if (rawDice) {
            const parsed = JSON.parse(rawDice);
            diceGames = parsed;
        }
    } catch(e) {
        diceGames = {};
